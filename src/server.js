...
app.get('/ping', signature_required, (req, res) => {
    ...
    rc.multi()
        .llen(`bq:${QUEUE_NAME}:waiting`)
        .scard(`bq:${QUEUE_NAME}:failed`)
        .exec((err, results) => {
...
app.post('/download', signature_required, (req, res) => {
    ...
    if (
        isObject(data) === true &&
        data.hasOwnProperty('uifnKey') &&
        data.hasOwnProperty('site') &&
        data.hasOwnProperty('board_id') &&
        data.hasOwnProperty('uifn') &&
        data.hasOwnProperty('board_pins') &&
        data.hasOwnProperty('etime') &&
        data.hasOwnProperty('MAX_BOARD_NUMBER') &&
        data.hasOwnProperty('CALLBACK_URL')
    ) {
        let uifn = data.uifn
        ...
        rc.multi()
            ...
            .exec((err, results) => {
                if (!err) {
                    ...
                    job.timeout(Number(data.TIMEOUT || 7200) * 1000)
                        .retries(0)
                        .save()
                        .then(job => {
